...
A name has to be a shell identifier (`[A-Za-z_][A-Za-z0-9_]*`) — `mien use` writes a script that gets sourced, and a name the shell won't take breaks the loader — and it may not be one of the nineteen variables mien already uses for a built-in service. Nor may it be a variable the shell or mien itself reads as an instruction rather than carries as payload: `PATH`, `HOME`, `IFS`, `PS1`, `TMPDIR`, `MIEN_CONFIG`. Because the scrub list is the union over *all* profiles, one such name in one profile would make every `mien use` and every `mien-unset`, in every shell, strip it — a name in a profile you never activate would wreck shells that have nothing to do with it. The match is exact, so `MY_PATH` and `PATH_TO_KEY` are fine. All of it is refused when you log in *and* when the config is parsed, so a hand-edit fails the same way; the collision error names the service it would have fought, and the shell-critical error says what the variable is for.
