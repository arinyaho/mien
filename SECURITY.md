...
**Keeping secrets out of transcripts, shell history, and `ps`.** `mien use` never prints a secret; it writes the exports to a mode-0600 file and prints a one-liner that sources and deletes it. On a real terminal it refuses to run at all unless you pass `--print`. `mien login` reads secrets through a hidden prompt, a `--secret-cmd` reference, or stdin. Slack is stricter: the raw token is delivered only through the mode-0600 JSON map at `MIEN_SLACK_TOKENS`; a single-workspace profile exports the non-secret `MIEN_SLACK_DEFAULT_WORKSPACE`, not a token. In a non-agent terminal only, `MIEN_SLACK_LEGACY_DEFAULT_TOKEN=1` restores the old raw `MIEN_SLACK_DEFAULT_TOKEN` for compatibility. Any agent marker overrides that opt-in.

Every resolved secret is registered with the diagnostic redactor. Mien removes known environment credential values, Authorization header values, JSON credential-map values, and known secrets found in exception arguments. Under a detected agent harness, `mien exec` and `mien run` also capture and redact the child process's stdout and stderr before returning it to the harness, preventing a child traceback from replaying a token it read from a credential file. This is a last-resort output boundary, not permission to print credentials intentionally.

*Not protected:* `mien token <service>` prints a token to stdout by design — that is its purpose — and for Atlassian and Notion that token is long-lived, not a short-lived access token. Whatever captures that stdout holds the secret. Because the usual way that goes wrong is accidental — piping the token into a program that echoes its input, so a traceback or usage error writes the secret into a session log — `mien token` refuses by default when it detects an agent harness that records output — Claude Code through `$CLAUDECODE` or `$CLAUDE_CODE_ENTRYPOINT`, Codex through `$CODEX_THREAD_ID`, and GitHub Copilot Chat through `$COPILOT_AGENT` — and names `mien exec <profile> -- <cmd...>`, which passes the credential in the environment instead. The detection is a heuristic and fails *open*: an unrecognized harness is not detected unless you mark it yourself by setting `MIEN_CAPTURED=1`, which triggers the same refusal in any harness. Every refusal is overridable (`MIEN_TOKEN=capture-ok`, `--force`), so it guides rather than traps, and it changes nothing for a person at a terminal. It reduces accidents; it does not make the value unreadable — see the next paragraph.
...
`mien exec` blocks in that same direction, and only in that direction. When an agent harness is driving the call — detected exactly as `mien token`'s capture refusal detects it — and the profile named on the command line contradicts the profile this place claims by `owns_remotes` or `default_for`, the handover is refused before any credential is loaded. The repo signal still cannot *choose* an identity here: it can only veto the one that was named explicitly, so a crafted `origin` buys a false refusal and never a mis-routed credential. This gate is fail-open on every uncertainty and on any internal error too — with one exception: an `origin` whose authority cannot be confidently parsed at all (a password with an unencoded `/`, `?` or `#`, indistinguishable by shape from an ordinary path, query, or fragment that happens to contain an `@` anywhere) also refuses, because nothing safely guesses the real owner from that shape either way — several attempts to do so each misattributed some other, ordinary remote to the wrong configured owner about as often as they recovered a genuinely truncated one. This is still a refusal, never a selection: it blocks a *named* mismatch, the same asymmetry as every other case here, so a crafted `origin` still buys at most a false refusal. It never fires for a person at a terminal. It has no per-call `--force`, on the reasoning that an override the agent can see is an override the agent will use; `MIEN_EXEC=off` exists for a human debugging a false refusal and is documented in the README rather than named in the refusal.
...
Variables that carry a **secret value directly**: `GH_TOKEN`, `AWS_ACCESS_KEY_ID`, `AWS_SECRET_ACCESS_KEY`, `ATLASSIAN_API_TOKEN`, `NOTION_TOKEN`, and every variable named in a profile's `custom` block (that is what the block is for — one name you chose, one secret from the backend). `MIEN_SLACK_DEFAULT_TOKEN` joins this group only in a non-agent terminal with the explicit `MIEN_SLACK_LEGACY_DEFAULT_TOKEN=1` compatibility opt-in.
...
Variables that carry **only a selector**: `MIEN_PROFILE`, `MIEN_EPHEMERAL_DIR`, `CLOUDSDK_ACTIVE_CONFIG_NAME`, `CLOUDSDK_CORE_PROJECT`, `MIEN_SLACK_DEFAULT_WORKSPACE`, `AWS_PROFILE`, `AWS_DEFAULT_REGION`, `OCI_CLI_PROFILE`, `OCI_CLI_CONFIG_FILE`, `ATLASSIAN_EMAIL`, `ATLASSIAN_BASE_URL`.
