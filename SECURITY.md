...
*Not protected:* `mien token <service>` prints a token to stdout by design — that is its purpose — and for Atlassian and Notion that token is long-lived, not a short-lived access token. Whatever captures that stdout holds the secret. Because the usual way that goes wrong is accidental — piping the token into a program that echoes its input, so a traceback or usage error writes the secret into a session log — `mien token` refuses by default when it detects an agent harness that records output — the built-in markers (`$CLAUDECODE`, `$CLAUDE_CODE_ENTRYPOINT`) are Claude Code's, and no others are recognized — and names `mien exec <profile> -- <cmd...>`, which passes the credential in the environment instead. The detection is a heuristic and fails *open*: an unrecognized harness is not detected unless you mark it yourself by setting `MIEN_CAPTURED=1`, which triggers the same refusal in any harness. Every refusal is overridable (`MIEN_TOKEN=capture-ok`, `--force`), so it guides rather than traps, and it changes nothing for a person at a terminal. It reduces accidents; it does not make the value unreadable — see the next paragraph.
