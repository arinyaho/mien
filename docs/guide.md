...
Like `guard`, it fails open on nearly every uncertainty: no config, nothing claiming this place, an ambiguous claim, no repository or no remote, or any internal error all allow the handover. The one exception is an `origin` whose authority can't be confidently parsed at all — a password with an unencoded `/`, `?` or `#` — where nothing claims the place *and* the origin is that shape refuses instead: several attempts to guess the real owner from it each turned out to misattribute some other, ordinary remote about as often as they recovered a genuinely malformed one, so this refuses rather than guesses (see [SECURITY.md](../SECURITY.md#protection-goals-and-what-is-not-protected)). For a false refusal you need to get past while you fix the underlying config, set `MIEN_EXEC=off` (also `0`, `false`, `no`) — the same convention as `MIEN_GUARD`. It is documented here and not in the error text, because the human who needs it reads the docs and the agent that must not have it reads the error.
