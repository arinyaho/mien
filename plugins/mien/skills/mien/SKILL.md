...
- `mien guard` — the **enforcement**, and it is harness-agnostic: as a git pre-commit hook it blocks a mis-authored commit in *any* session (Claude Code, Codex, Copilot Chat, a plain shell), and it is the strongest of the three guarantees. Prefer it when the goal is prevention rather than display.
...
**Your shell state does not survive between tool calls.** Claude Code, Codex, Copilot Chat, and most
...
- it is one of the agent-harness capture markers mien reads to know an agent is driving: `CLAUDECODE`, `CLAUDE_CODE_ENTRYPOINT`, `CODEX_THREAD_ID`, `COPILOT_AGENT`, `MIEN_CAPTURED`.
