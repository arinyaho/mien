...
A template you do supply must use **every token mien renders it with** — `{profile}`, `{service}` and `{kind}` for `default`; `{profile}` and `{workspace}` for `slack_token` (not `{service}`, since the literal `slack` is the template's own text). Arrangement and surrounding text are yours; presence is not. A missing token is a `ConfigError` naming which credentials would collapse, because dropping one silently maps two credentials onto one backend secret: the second `mien login` overwrites the first with nothing said, and a later `mien logout` of either deletes the secret the other still points at. Drop `{kind}` and every credential of one service collides — including every `--service custom` variable; drop `{service}` and atlassian's token collides with notion's; drop `{profile}` and two identities share one secret, which is the one thing mien exists to keep apart.

Presence is checked by parsing the template, not by searching its text, so `{{kind}}` (an escaped literal that substitutes nothing) and `{kind[0]}` (one character, so two kinds sharing a first letter still collide) are both refused. A template that is not parseable at all is a `ConfigError` too rather than a bare `ValueError`, since the fail-open surfaces recognize only `ConfigError`.
