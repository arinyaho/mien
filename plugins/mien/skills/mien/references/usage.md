...
Two limits worth knowing before you rely on the exit code as a gate: **AWS is reported, not verified** — a profile name is not an ARN, so there is no configured value to compare and a wrong-but-valid AWS account will not trip the gate; and **GitHub, AWS and Google are the only services with a live probe at all**, so everything else the profile configures is listed by name under `not checked (no live probe yet)` rather than pretended verified. `custom` appears there — mien is told a variable name, never what the credential is for — and so does a gcloud-login-only `google` with no stored refresh token, which the probe structurally cannot verify. Read the line rather than assuming a clean report covered the service you care about; the gate itself is trustworthy for GitHub and Google. One more limit before you chain it: `--live` needs at least one of github/aws/google on the profile to run at all — with none it exits non-zero with `profile '<p>' has no provider that supports live verification (github, aws, or google)` and prints no report, so a `custom`-only or notion-only profile fails the `&&` without having checked anything. It fails closed, so nothing runs as the wrong identity, but read that failure as "could not check" rather than "wrong identity", and gate those profiles with the inline comparison below or the service's own check. Chain it before anything you cannot take back:
